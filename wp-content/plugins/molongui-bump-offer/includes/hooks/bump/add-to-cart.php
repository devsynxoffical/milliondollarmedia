...
function mbo_ajax_add_to_cart()
{
    ...
    if ( $passed_validation && $cart_item_key && 'publish' === $product_status )
    {
        ...
        (
            ...
            'added_item_key' => $cart_item_key,
        );
        if ( ! headers_sent() )
        {
            mbo_setcookie(
                'molongui_added_order_bump_' . $deal_id,
                $cart_item_key,
                0,
                '/',
                '',
...
function mbo_overwrite_deal_price( $cart_object )
{
    ...
    if ( ! WC()->session || WC()->session->__isset( 'reload_checkout' ) )
    {
        return;
    }

    $is_wc_3_or_newer = version_compare( WC()->version, '3.0.0', '>=' );
    foreach ( $cart_object->get_cart() as $key => $value )
    {
        if ( isset( $value['order_bump_price'] ) && isset( $value['data'] ) && is_object( $value['data'] ) )
        {
            if ( ! $is_wc_3_or_newer )
            {
...
function mbo_overwrite_deal_price_minicart( $price, $cart_item, $cart_item_key )
{
    ...
    if ( !empty( $cart_item['order_bump_price'] ) )
    {
        return wc_price( (float) $cart_item['order_bump_price'] );
    }
